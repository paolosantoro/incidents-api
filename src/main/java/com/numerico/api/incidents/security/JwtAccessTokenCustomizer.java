...
implements JwtAccessTokenConverterConfigurer {
	...
	private static final Set<String> USER_ATTRIBUTES_ELEMENTS = Sets.newHashSet("customerId", "region");
	...
	public OAuth2Authentication extractAuthentication(Map<String, ?> tokenMap) {
		...
		Map<String, Object> userAttributes = extractUserAttributes(token);
		
		Authentication usernamePasswordAuthentication = 
				new KeycloakAuthenticationToken(authentication.getPrincipal(), 
						"N/A", userAttributes, authorities);
	...
	private Map<String, Object> extractUserAttributes(JsonNode jwt) {
		return USER_ATTRIBUTES_ELEMENTS.stream()
				.map(element -> new AbstractMap.SimpleImmutableEntry<String, Object>(element, jwt.path(element).asText()))
